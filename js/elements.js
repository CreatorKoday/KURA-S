// ==========================================================
// よく参照するDOM要素をまとめて取得しておく(複数モジュールから共有)
// ==========================================================

export const authCard = document.getElementById("auth-card");
export const loggedInArea = document.getElementById("logged-in-area");
export const messageBox = document.getElementById("message");
export const addMessageBox = document.getElementById("add-message");
export const manualAddMessageBox = document.getElementById("manual-add-message");
export const consumeMessageBox = document.getElementById("consume-message");
export const manualConsumeMessageBox = document.getElementById("manual-consume-message");
export const reviewMessageBox = document.getElementById("review-message");
export const shoppingMessageBox = document.getElementById("shopping-message");
export const userEmailLabel = document.getElementById("user-email");
export const itemListEl = document.getElementById("item-list");
export const shoppingListEl = document.getElementById("shopping-list");
